  ...
  <xsl:template match="persname">
    <tr class="medalRow">
      ...
      <td class="medalplain" width="35%">
        <xsl:value-of disable-output-escaping="yes" select="emph[@altrender='surname']/text()"/>
        <xsl:if test="emph[@altrender='surname'] and emph[@altrender='forenames']">
          <xsl:text disable-output-escaping="yes">, </xsl:text>
        </xsl:if>
        <xsl:value-of disable-output-escaping="yes" select="emph[@altrender='forenames']/text()"/>
      </td>
    </tr>
  </xsl:template>
  <!-- add petitioners -->
