<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template mode="deathduty" match="emph[@altrender='doctype']">
		<xsl:choose>
	...
	<!-- show title and " "-->
	<xsl:template mode="deathduty" match="emph[@altrender='perstitle']">
			<xsl:value-of select="text()"/>
			<xsl:text> </xsl:text>
	</xsl:template>

	<!-- show firstname " " lastname -->
	<xsl:template mode="deathduty" match="persname">
	...
	<!-- add ", " then occupation -->
	<xsl:template mode="deathduty" match="occupation">
			<xsl:text>, </xsl:text>
			<xsl:value-of select="text()" />
	</xsl:template>

	<!-- add " of " then name of place -->
	<xsl:template mode="deathduty" match="geogname">
